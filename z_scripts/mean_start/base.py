...
class DataPrep:
    ...
    def calc_cp_pure(self):
        ...
        for i in range(len(df_pure) - 1):
            # heatflow per temperature step (HARDCODED TO 0.1K/MIN SETTING)
...
class DataCP:
    ...
    def plot_data(self,colour,raw_data=1,mean_data=1,shomate=1,errors=1,ci=1,melt=1,pure=1):
        # manually control where labels dependent on melt/pure arguments
        fig = plt.figure()
        ax = plt.gca()
        if melt == 1:
            if raw_data == 1:
                plt.scatter(self.df_m['T(K)'], self.df_m['cp(J/gK)'], 1, c=colour, marker='o')
            if mean_data == 1:
            ...
            if shomate==1:
                plt.plot(self.df_m['T(K)'], shomate_eqn(self.df_m['T(K)'], *self.shomate_m), c=colour
                     ,label='{} wt%'.format(self.wt))
            if errors==1:
                plt.errorbar(self.df_m['T(K)'], self.df_m['cp(J/gK)'], yerr=(self.df_m['cp_err(%)']/100*self.df_m['cp(J/gK)']),
                             linestyle='', ecolor=colour, capsize=1, capthick=0.5, elinewidth=0.5,errorevery=20)
            if ci==1:
                lower = shomate_eqn(self.df_m['T(K)'], *(self.shomate_m-self.sd_m*2)).to_numpy()
                upper = shomate_eqn(self.df_m['T(K)'], *(self.shomate_m+self.sd_m*2)).to_numpy()
                plt.plot(self.df_m['T(K)'], lower, c=colour
                     ,label='{} wt%'.format(self.wt))
                # ax.fill_between(self.df_m['T(K)'],lower,upper)

        if pure == 1:
            if raw_data == 1:
                plt.scatter(self.df_p['T(K)'], self.df_p['cp(J/gK)'], 1, c=colour, marker='o')
            if mean_data ==1:
            ...
            if errors == 1:
                plt.errorbar(self.df_p['T(K)'], self.df_p['cp(J/gK)'], yerr=(self.df_p['cp_err(%)']/100*self.df_p['cp(J/gK)']),
                             linestyle='', ecolor=colour, capsize=1, capthick=0.5, elinewidth=0.5,errorevery=20)
