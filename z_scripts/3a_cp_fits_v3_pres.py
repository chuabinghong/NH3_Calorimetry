    ...
    def fun(ax):
        ...
        if lit_arg ==2:
            ...
            plt.scatter(df_wrewsky['T(K)'], df_wrewsky['cp(J/kgK)'] / 1000, 15, marker='P', edgecolors='k',
                        linewidths=.5, c=df_wrewsky['massfrac']*100, cmap=cmap)
            plt.scatter(df_chernenkaya['T(K)'], df_chernenkaya['cp(J/kgK)'] / 1000, 15, marker='X', edgecolors='k',
                        linewidths=.5,c=df_chernenkaya['massfrac']*100, cmap=cmap)
...
def label_one(layout):
    ...
    for i, wt in enumerate(wt_ls):
        ...
        plt.plot(240, 4, linestyle=(0, (3, 1, 1, 1)),c=colour_ls[i],alpha=0, label=f'{wt} wt% TF98')
    ...
    if layout == 'hori':
        ...
        ncol = 1
        loc = 'center left'
    leg = plt.legend(loc='lower right',prop={'size': 4}, ncol=ncol)
...
# overall_fun(ax,1,0,1,1,0,0,0,1,0)
overall_fun(ax,1,1,1,0,0,0,0,0,0)


label_one(layout)
plt.savefig(fd + 'z_pres_TRF.png')
