    ...
    def fun(ax):
        ...
        if best_fit_arg==2:
            ...
            if pure_arg==0:
                ...
                ax.scatter(this['T(K)'], this['cp(J/kgK)'] / 1000, 15, c=matplotlib.colors.rgb2hex(cmap(this['massfrac']*100)), marker='s', edgecolors='k',
...
def label_two():
    ...
    plt.plot(250, 4, 'k', linestyle=(0, (3, 1, 1, 1, 1, 1)), linewidth=1,label='Overall Fit')

    handles, labels = plt.gca().get_legend_handles_labels()
    order = [ 3, 4, 5, 6, 7, 8, 9, 0, 1, 10,2]
    # order = [2,3,4,5,6,7,8,9,0,1,12,10,11]
...
##
w = 3.3
fig, ax = plt.subplots(2, 1, figsize=(w, w * 3 / 3))
overall_fun(ax[0],0,0,0,0,1,2,1,1,0,)
...
for i, wt in enumerate(wt_ls):
    ...
    if wt == 20.07:
        ...
        wt_str = 26.9
    ax[0].scatter(210, 3, marker=marker_ls[i], c=colour_ls[i], linewidths=0, alpha=0, label=f'{wt_str} wt%')
ax[0].plot(210, 3, linewidth=1, c=colour_ls[0], label='Quadratic Fit')
ax[0].plot(210, 3, "--", linewidth=.5, color='gray', label='95% Prediction Limit')
...
leg = ax[0].legend([handles[idx] for idx in order], [labels[idx] for idx in order],
                 prop={'size': 3.5}, title='Starting Mass Fraction', title_fontsize=5, ncols=2)
for lh in leg.legendHandles:
    lh.set_alpha(1)
...
overall_fun(ax[1],1,0,1,0,1,2,0,0,0,)
