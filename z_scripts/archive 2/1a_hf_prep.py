...
wt_arg = 2 # 0 for lab measurements, 1 for liquidus alignment, 2 for liquidus alignment -0.5K
...
if smallHF_arg == 1:
    ...
    fd = r"o_heatFlow/46C/"
    mass_ls = [.9642]
    # wt_ls = [2.84] # based off lab measurements
    wt_ls = [2.9] # based off liquidus alignment
...
for idx, wt in enumerate(wt_ls):
    idx = 2 #TODO for single data debugging
    ...
    #
    # plt.plot(data.df2['Sample Temperature(K)'], data.df2['Q_Corrected'],label=f'{wt}wt%')
    # plt.xlabel("Temperature (K)");
    # plt.ylabel('Heat Flow (mW)')
    # plt.title('{} wt% Thermogram'.format(data.wt))
    # plt.ylim([-35.5,-29])
    # plt.xlim([275,315])
    # plt.show()
