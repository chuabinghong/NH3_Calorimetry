...
lowC_ls = [180,180,200,220,200,300]
highC_ls = [-1150,-1240,-1000,-600,-720,-1]

file_ls = [dd+f'{i}wt%_cp.csv' for i in wtpct_ls]
fileL_ls =[dd+f'{i}wt%_cp_liquidPhase.csv' for i in wtpct_ls]
...
## TEST LOW T CUTOFF
    ...
    if samp ==6.88:
        df_c = df_c.drop(range(1630,1800))
    if samp == 7.85:
        df_c = df_c.drop(range(1630, 1800))
    plt.close()
    fig = plt.figure(figsize=(3,2))
    ...
    plt.legend(markerscale=5)
    # plt.show()
    saveFig = fd + '{}wt%_cp_cut.png'.format(samp)
    plt.savefig(saveFig)
    plt.close()

## SAVE

    saveFile = od + '{}wt%_cp_cut.csv'.format(samp)
