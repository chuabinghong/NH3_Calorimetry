...
if test_toolbox == 1
    switch material
        ...
        case 'NH3'
            if PT{1,3} == 0 % use water1, which goes to a higher pressure
            load('SeaFreeze_Gibbs.mat', 'G_H2O_2GPa_500K');
            MW_H2O = 1e-3*18.01528;
            G_H2O_2GPa_500K.MW = MW_H2O;
            G_H2O_2GPa_500K.nu = 1;
            G_H2O_2GPa_500K.Go = 1;
             out=fnGval(G_H2O_2GPa_500K,PT);
            else
            load('LBF_2_2p3GPa'); 
%             load('NH3H2O_1_3'); % as of March 1, 2022, goes to 800 MPa
%             load('NH3_LBF_V8'); 
            out=fnGval(LBF_NH3_H2O_SSdev_v1,PT);
%             out=fnGval(sp_114,PT);
            end
    end
else
    switch material
