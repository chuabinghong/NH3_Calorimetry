...
# surf = mlab.plot_surface(Y, X, trf/1000,color='gray',linewidth=0, antialiased=False,alpha=.5)
surf = mlab.surf(Y, X, trf/1000,opacity=.5)
...
for idx, wt in enumerate(wt_ls):
    ...
    mlab.points3d(data['T(K)'],np.repeat(wt,len(data)),data['cp(J/gK)'],color='#225ea8')
...
# PLOT LITERATURE
mlab.scatter(df_chan['T(K)'],df_chan['massfrac']*100,df_chan['cp(J/kgK)']/1000,s=5,marker=marker_ls[0],label= 'CG64',edgecolors='k', linewidths=.5,alpha=1)
mlab.scatter(df_hildenbrand['T(K)'],df_hildenbrand['massfrac']*100,df_hildenbrand['cp(J/kgK)']/1000,s=5,marker=marker_ls[1],label= 'HG53',edgecolors='k', linewidths=.5,alpha=1)
mlab.scatter(df_wrewsky['T(K)'],df_wrewsky['massfrac']*100,df_wrewsky['cp(J/kgK)']/1000,s=5,marker=marker_ls[2],label= 'WK24',edgecolors='k', linewidths=.5,alpha=1)
mlab.scatter(df_chernenkaya['T(K)'],df_chernenkaya['massfrac']*100,df_chernenkaya['cp(J/kgK)']/1000,s=5,marker=marker_ls[3],label= 'C71',edgecolors='k', linewidths=.5,alpha=1)
mlab.scatter(df_allred['T(K)'],df_allred['massfrac']*100,df_allred['cp(J/kgK)']/1000,s=5,marker=marker_ls[4],label= 'AW81',edgecolors='k', linewidths=.5,alpha=1)
mlab.scatter(df_fujita['T(K)'],df_fujita['massfrac']*100,df_fujita['cp(J/kgK)']/1000,s=5,marker=marker_ls[5],label= 'F08',edgecolors='k', linewidths=.5,alpha=1)
...
ax.view_init(elev=0, azim=315, roll=0)
plt.show()

angle = 3
ani = animation.FuncAnimation(fig, rotate, frames=np.arange(0, 360, angle), interval=50)
ani.save(fd+ '3d.gif', writer=animation.PillowWriter(fps=20))
